...
export default class Releases {
    ...
    public create(options: IReleaseOptions): Promise<IObject> {
        if (options.released) {
            options.releaseDate = this.formatDate(options.releaseDate ?? new Date().toISOString());
        } else {
            options.startDate = this.formatDate(options.startDate ?? new Date().toISOString());
        }
    ...
    public get(projectId: number, versionId: number): Promise<IObject> {
        const url = new URL(`${this.config.url}/rest/api/latest/version/${versionId}`);
        return this.httpservice.get(url);
    }
